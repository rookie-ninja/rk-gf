...
const (
	...
	StaticFileHandlerEntryDescription = "Internal RK entry which implements static file handler GoFrame framework."
)
...
func (entry *StaticFileHandlerEntry) GetFileHandler() ghttp.HandlerFunc {
	return func(ctx *ghttp.Request) {
		w := ctx.Response.Writer
		r := ctx.Request

		// Trim prefix with path user defined in order to get file path
		p := strings.TrimSuffix(strings.TrimPrefix(r.URL.Path, strings.TrimSuffix(entry.Path, "/")), "/")
		if len(p) < 1 {
			p = "/"
		}
		p = path.Join("/", p)
