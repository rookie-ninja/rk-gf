...
func readFileFromPkger(filePath string) []byte {
	...
	if file, err = pkger.Open(path.Join("github.com/rookie-ninja/rk-gf:/boot", filePath)); err != nil {
		return []byte{}
	}
...
func (entry *TvEntry) AssetsFileHandler() ghttp.HandlerFunc {
...
// @Router /rk/v1/tv/ [get]
func (entry *TvEntry) TV(ctx *ghttp.Request) {
	if ctx == nil {
		return
	}
