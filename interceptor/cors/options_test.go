...
func TestNewOptionSet(t *testing.T) {
	// With empty option
	set := newOptionSet()
	assert.Equal(t, rkgfinter.RpcEntryNameValue, set.EntryName)
	assert.Equal(t, rkgfinter.RpcEntryTypeValue, set.EntryType)
	...
	set = newOptionSet(
		...
		WithSkipper(func(context *ghttp.Request) bool {
			return true
		}),
