...
func TestNewOptionSet(t *testing.T) {
	...
	assert.False(t, set.Skipper(&ghttp.Request{}))
	...
	set = newOptionSet(
		...
		WithSkipper(func(request *ghttp.Request) bool {
			return true
		}),
		WithTokenLength(10),
	...
	assert.True(t, set.Skipper(&ghttp.Request{}))
...
func TestCsrfTokenFromHeader(t *testing.T) {
	...
	res, err := set.extractor(&ghttp.Request{
		Request: req,
	})
	assert.Nil(t, err)
